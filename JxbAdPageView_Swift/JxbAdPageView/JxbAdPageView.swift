...
class JxbAdPageView: UIView,UIScrollViewDelegate {
    ...
    private func showImage() {
        ...
        var namePre:String = imagesList.objectAtIndex(indexPre) as! String;
        
        var nameCur:String = imagesList.objectAtIndex(indexShow) as! String;
        
        var indexNext:NSInteger = indexShow + 1;
        if (indexNext >= imagesList.count) {
            indexNext = 0;
        }
        var nameNext:String = imagesList.objectAtIndex(indexNext) as! String;
        ...
        scView.scrollRectToVisible(CGRectMake(scView.frame.width, 0, scView.frame.width, scView.frame.height), animated: false);
    }
...
/*-------public-----------*/
    ...
    func timerAction() {
        scView.scrollRectToVisible(CGRectMake(scView.frame.width*2, 0, scView.frame.width, scView.frame.height), animated: true);
        indexShow++;
